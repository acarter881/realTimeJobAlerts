...
class myWorkday:
    def __init__(self):
        self.url = 'https://www.REDACTED.com/REDACTED/task/1422$2354.htmld'
        self.jobGroupToFind = '4'
        self.workDayUser = 'REDACTED@REDACTED.com'
        self.workDayPass = os.environ.get('WORKDAY_PASS')
    ...
    def scrapeHTML(self):
        ...
        try:
            self.myJSON = json.loads(self.soup.body.find('pre').next)                                   # In rare cases, will give an AttributeError: 'NoneType' object has no attribute 'next'
        ...
        if len(self.jobURLS) == 0:
            ...
            for i in tqdm(range(len(self.jobURLS))):
                ...
                try:
                    ...
                    self.jobGroup = 'Can\'t find the JG in the HTML'

                try:
                    self.hiringManager = self.soup.find(id='promptOption-gwt-uid-14').text
                except Exception as e:
                    self.hiringManager = 'Could not find the hiring manager in the HTML.'
                ...
                if self.jobGroup == self.jobGroupToFind:
                    ...
                    self.validHiringManagers.append(self.hiringManager)
                else:
                    pass
    ...
    def myToaster(self):
        if len(self.validJobs) == 0:
            ...
            for i in range(len(self.validJobs)):
                ...
                # playsound(sound='newJob.mp3')
                # time.sleep(3)
                self.toaster.show_toast(title='A New Job Has Been Posted!', msg=f'Title: {self.validTitles[i]}\nLocation: {self.validLocations[i]}\nJob Group: {self.jobGroupToFind}\nHiring Manager: {self.validHiringManagers[i]}', icon_path=r"C:\Users\REDACTED\Desktop\Documents\Miscellaneous\work.ico", duration=25)
...
while True:
    ...
    WORKDAY.scrapeHTML()
    WORKDAY.toDatabase(r'C:\Users\REDACTED\myIPYNBs\Workday.db')
    WORKDAY.findOurJobs()
    WORKDAY.myToaster()
    #WORKDAY.sendTheMail() 
    runs += 1            # Increment the run count by 1
    time.sleep(600)      # Amount of time (in seconds) to wait between runs
